...
export class OutcomeVerifier {
  ...
  async verify(goal: string, page: PageSnapshot, history: ActionRecord[], evidence: TaskEvidence[] = []): Promise<Verification> {
    ...
    log('verification_start', { inputChars: prompt.length, rawInputChars, evidenceCount: evidence.length });
    let onAbort!: () => void;
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        try {
          ...
          log('verification_error', { attempt, outcome: this.signal?.aborted ? 'cancelled' : 'error', ...details, ...(malformed ? { finishReason: error.finishReason, generatedChars: error.text?.length ?? 0, usage: error.usage } : {}) });
          const delayMs = 250;
