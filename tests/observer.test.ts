...
test("observer retries until document body becomes available", async () => {
  ...
  const api = {
    ...
    sendCommand: async () => ({ result: { value: ++calls === 1 ? null : raw } }),
  };
...
test("waits through delayed updates until changed page settles", async () => {
  ...
  const observer = new CdpObserver({ attach: async () => {}, detach: async () => {}, sendCommand: async () => ({ result: { value: { ...raw, text: ++samples >= 4 ? "loaded" : "before" } } }) }, { tabId: 1 }, async () => { pauses++; });
...
test("unchanged pages have bounded wait", async () => {
  ...
  const observer = new CdpObserver({ attach: async () => {}, detach: async () => {}, sendCommand: async () => ({ result: { value: raw } }) }, { tabId: 1 }, async () => { pauses++; });
...
test('layout churn cannot end waiting before a delayed control arrives', async () => {
  ...
  const observer = new CdpObserver({ attach: async () => {}, detach: async () => {}, sendCommand: async () => ({ result: { value: { ...raw, diagnostics: { candidates: samples++ < 4 ? 1 : 2 }, text: samples >= 18 ? 'Search ready' : 'Loading' } } }) }, { tabId: 1 }, async () => { pauses++; });
...
test('explicit wait stays bounded on unchanged content and cancels between samples', async () => {
  ...
  const raw = { pageIdentity: 'p', url: 'https://example.test', title: '', text: '', scroll: { y: 0, height: 100, viewportHeight: 100 }, elements: [], guards: {} };
  const observer = new CdpObserver({ attach: async () => {}, detach: async () => {}, sendCommand: async () => ({ result: { value: raw } }) }, { tabId: 1 }, async () => { pauses++; if (pauses === 42) cancelled = true; });
  const before = await observer.observe();
